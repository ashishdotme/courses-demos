...
function App() {
  ...
  const getNews = async () => {
    ...
    try {
      const results = await axios.get(`http://hn.algolia.com/api/v1/search?query=${query}`)
  ...
  return (
    <div className="container max-w-md mx-auto p-4 m-2 bg-purple-100 shadow-lg rounded">
      ...
      <form onSubmit={handleSubmit} className="mb-2">
        ...
        <button type="button" onClick={clearSearch} className="bg-teal-500 text-white p-2 rounded">
          Clear
        </button>
      </form>
        ...
        <ul>
          {results.map((result, i) => (
            <li className="list-reset leading-normal" key={i}>
              <a href={result.url} className="text-indigo-600 hover:text-indigo-900">
                {result.title}
              </a>
            </li>
          ))}
